...
@st.cache_resource
def load_model():
    data = joblib.load("lahore_house_price_model.pkl")
    return data["model"], data["columns"]

model, model_columns = load_model()

# -------------------------
# Extract location list from training columns
# -------------------------
...
types = sorted([c.replace("Type_", "") for c in type_cols]) + ["House"]  # House was dropped as base
